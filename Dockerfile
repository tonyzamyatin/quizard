...
# Copy the backend directory contents (our code) into the container at /app
COPY ./backend .

# Install dependencies and clearing useless Poetry cache
# --without dev: Don't install development dependencies (useless, only slow down installation)
...
ENTRYPOINT ["poetry", "run", "python", "-m", "src.rest.app"]