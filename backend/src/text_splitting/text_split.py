...
def calculate_window_size(base_prompt_size, config):
    if config["model"]["name"] == "gpt-3.5-turbo" and base_prompt_size < config["tokens"]["4k_model"]["base_prompt_limit"]:
        return 4000 - base_prompt_size - config["tokens"]["4k_model"]["completion_limit"]
    elif base_prompt_size < config["tokens"]["16k_model"]["base_prompt_limit"]:
        return 16000 - base_prompt_size - config["tokens"]["16k_model"]["completion_limit"]
    else:
